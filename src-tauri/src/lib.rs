...
fn guardar_todo_en_disco(estado: Value) -> Result<String, String> {
    let contenido = serde_json::to_string_pretty(&estado)
        .map_err(|e| e.to_string())?; // Mantenemos tu forma original
    
    fs::write("../datos_asistente.json", contenido)
...
fn cargar_todo_desde_disco() -> Result<Value, String> {
    let contenido = fs::read_to_string("../datos_asistente.json")
        .map_err(|e| e.to_string())?;
...
async fn exportar_datos(handle: tauri::AppHandle) -> Result<String, String> {
    ...
    let ruta_destino = handle.dialog()
        .file()
    ...
    if let Some(ruta) = ruta_destino {
        fs::write(ruta.to_string(), contenido)
            .map_err(|e| e.to_string())?;
...
async fn importar_datos_nativa(handle: tauri::AppHandle) -> Result<Value, String> {
    ...
    let archivo_seleccionado = handle.dialog()
        .file()
    ...
    if let Some(ruta) = archivo_seleccionado {
        let contenido = fs::read_to_string(ruta.to_string())
            .map_err(|e| e.to_string())?;
...
pub fn run() {
  tauri::Builder::default()
    ...
    .invoke_handler(tauri::generate_handler![guardar_todo_en_disco, cargar_todo_desde_disco, exportar_datos, importar_datos_nativa])
    .setup(|app| {
